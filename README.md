...
**每周自動把蝦皮台灣賣場所有商品的「備貨天數」改為 1 天**。

驅動蝦皮賣家中心自帶的「更新商品資料」批次工具，完整跑通：
**登入 → 下載備貨天數模板（zip，內含多個 xlsx）→ 把每個 xlsx 的備貨天數改 1 → 逐個上傳**。

實測發現蝦皮這 5 個批次 API **只要 cookie、不需反爬簽名頭**，所以核心流程全部走純 API（不用點 UI、不用 input.files），穩定且快。

## 兩種版本

| 版本 | 路徑 | 適用 | 登入 |
|---|---|---|---|
| **Chrome 擴充功能** | [`extension/`](extension/) | 分發給別人、一鍵自動 | 瀏覽器自動（SPC_F + 表單） |
| **Python 腳本** | [`script/`](script/) | 自己用、可排程(cron) | 匯出完整 cookie（`cookies.json`） |

> ⚠️ 僅供個人自動化管理自己的賣場。請遵守蝦皮服務條款，自行承擔風險。
